...
# get place by name
def get_place_by_name(session: Session, name: str):
    place_model = session.query(place_models.Place).filter(place_models.Place.name == name).first()
    if not place_model:
        return None
    return place_model
